...
def choose_action(args, env, state, Q, evaluate = False):
    if not evaluate and np.random.uniform(0, 1) < args.epsilon:
        ...
        # action = np.argmin(Q[state[1],state[0], :])
    return action
 
#Function to learn the Q-value
def update(args, state, state2, reward, action, Q, alpha):
    predict = Q[state[1], state[0], action]
    # target = reward + args.gamma*Q[state2[1], state2[0], np.argmin(Q[state2[1],state2[0], :])]
    # target = reward + args.gamma*np.min(Q[state2[1], state2[0], :])
    target = reward + args.gamma*np.max(Q[state2[1], state2[0], :])
...
def train_agent(args, env, Q):
    ...
    # for _ in range(args.max_epsds):
        ...
        while t < args.max_stps:
            ...
            reward = find_reward(state1,state2) 
            ...
            Q[int(state1[1]), int(state1[0]), action1] = update(args, state1.astype(int), state2.astype(int), reward, action1, Q, a)
            state1 = state2.copy()
            t += 1
...
def initQ(args):
    ...
    if args.dontuseQ or not wr.qSARSA_exist(args, file_name):
        ...
        assert args.dontskipT, 'Q is not trained yet!'
        return Q
    else:
...
if __name__ == "__main__":
    ...
    if args.dontskipT:
        ...
        wr.write_np_arr(args, Q_learned, 'QLearning')
        wr.write_eps_reward(args, eps_reward, 'QLearning')
    else:
        Q_learned = Q
